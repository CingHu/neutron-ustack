...
class VPNPluginDb(vpnaas.VPNPluginBase, base_db.CommonDbMixin):
    ...
    def update_status(self, context, model, v_id, status):
        with context.session.begin(subtransactions=True):
            v_db = self._get_ipsec_resource(context, model, v_id)
            v_db.update({'status': status})

    def _get_ipsec_resource(self, context, model, v_id):
        try:
    ...
    def create_ipsec_site_connection(self, context, ipsec_site_connection):
        ...
        with context.session.begin(subtransactions=True):
            #Check permissions
            self._get_ipsec_resource(context,
                               VPNService,
    ...
    def update_ipsec_site_connection(
        ...
        validator = self._get_validator()
        with context.session.begin(subtransactions=True):
            ipsec_site_conn_db = self._get_ipsec_resource(
                context,
                IPsecSiteConnection,
                ipsec_site_conn_id)
    ...
    def _get_ipsec_site_connection(
        ...
        return self._get_ipsec_resource(
            context, IPsecSiteConnection, ipsec_site_conn_id)
    ...
    def update_ikepolicy(self, context, ikepolicy_id, ikepolicy):
        ...
        with context.session.begin(subtransactions=True):
            ...
            ike_db = self._get_ipsec_resource(context, IKEPolicy, ikepolicy_id)
            if ike:
                lifetime_info = ike.get('lifetime')
                if lifetime_info:
    ...
    def update_ipsecpolicy(self, context, ipsecpolicy_id, ipsecpolicy):
        ...
        with context.session.begin(subtransactions=True):
            ...
            ipsecp_db = self._get_ipsec_resource(context,
                                           IPsecPolicy,
                                           ipsecpolicy_id)
            if ipsecp:
    ...
    def delete_ipsecpolicy(self, context, ipsecpolicy_id):
        with context.session.begin(subtransactions=True):
            ...
            if ipsecpolicy:
                raise vpnaas.IPsecPolicyInUse(ipsecpolicy_id=ipsecpolicy_id)
            ipsec_db = self._get_ipsec_resource(context, IPsecPolicy, ipsecpolicy_id)
    ...
    def update_vpnservice(self, context, vpnservice_id, vpnservice):
        vpns = vpnservice['vpnservice']
        with context.session.begin(subtransactions=True):
            vpns_db = self._get_ipsec_resource(context, VPNService, vpnservice_id)
    ...
    def _get_vpnservice(self, context, vpnservice_id):
        return self._get_ipsec_resource(context, VPNService, vpnservice_id)
