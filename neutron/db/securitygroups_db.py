...
from neutron.extensions import _uos_sgrule_default_cfg as sg_cfg
...
class SecurityGroupDbMixin(ext_sg.SecurityGroupPluginBase):
    ...
    def __init_sg_db_mixin__(self):
        sg_cfg.DefaultSGRulesConfig.get_valid_rules()
    ...
    def _make_security_group_dict(self, security_group, fields=None,
        ...
        if process_extensions:
            self._apply_dict_extend_functions('security_groups', res,
                                              security_group)
    ...
    def create_security_group_rule_bulk_native(self, context,
        ...
        for rule_dict in r:
            ...
            #NOTE(gongysh) unspecified address should be None
            remote_ip_prefix = rule.get('remote_ip_prefix')
            if (remote_ip_prefix == "0.0.0.0/0" or
                remote_ip_prefix == "::/0"):
    ...
    def _validate_security_group_rules(self, context, security_group_rule):
        ...
        for rules in security_group_rule['security_group_rules']:
            ...
            if rule.get('remote_ip_prefix') and rule['remote_group_id']:
    ...
    def _ensure_default_security_group(self, context, tenant_id):
        try:
            return self.__ensure_default_security_group(context, tenant_id)
        except db_exc.DBDuplicateEntry as e:
            with excutils.save_and_reraise_exception() as ctxt:
                if e.columns == ['tenant_id', 'name']:
                    ctxt.reraise = False
                    return self.__ensure_default_security_group(context,
                                                                tenant_id)

    def __ensure_default_security_group(self, context, tenant_id):
        """Create a default security group if one doesn't exist.

        :returns: the default security group id. it should not be
        a subtransaction
        """
