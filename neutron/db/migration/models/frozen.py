...
#neutron/db/models_v2.py
class Port(BASEV2, HasId, HasTenant, TimestampMixin):
...
#neutron/db/models_v2.py
class Subnet(BASEV2, HasId, HasTenant, TimestampMixin):
    name = sa.Column(sa.String(255))
    network_id = sa.Column(sa.String(36), sa.ForeignKey('networks.id'))
...
#neutron/db/models_v2.py
class Network(BASEV2, HasId, HasTenant, TimestampMixin):
    name = sa.Column(sa.String(255))
    ...
    shared = sa.Column(sa.Boolean)
    uos_rate_limit = sa.Column(sa.Integer, default=600, nullable=False)
...
#neutron/db/l3_db.py
class Router(BASEV2, HasId, HasTenant, TimestampMixin):
    name = sa.Column(sa.String(255))
    status = sa.Column(sa.String(16))
    admin_state_up = sa.Column(sa.Boolean)
    ...
    enable_snat = sa.Column(sa.Boolean, default=True,
                            server_default=sa.sql.true(), nullable=False)
    ha_vr_id = sa.Column(sa.Integer, nullable=True)
    __table_args__ = (
        sa.UniqueConstraint('tenant_id', 'ha_vr_id',
                            name='uniq_l3router0tenant_id0ha_vr_id'),
    )


#neutron/db/l3_db.py
class FloatingIP(BASEV2, HasId, HasTenant, TimestampMixin):
    ...
    rate_limit = sa.Column(sa.Integer(), nullable=True, default=1024)
...
class RouterL3AgentBinding(BASEV2, HasId):
    ...
    __table_args__ = (
        sa.UniqueConstraint('router_id', 'l3_agent_id',
                   name='uniq_routerl3agentbind0router_id0l3_agent_id'),)
...
class SecurityGroup(BASEV2, HasId, HasTenant, TimestampMixin):
    __table_args__ = (
        sa.UniqueConstraint('tenant_id', 'name',
                            name='uniq_sg0tenant_id0name'),
    )
    name = sa.Column(sa.String(255))
    description = sa.Column(sa.String(255))
...
class VPNuser(BASEV2, HasId, HasTenant, TimestampMixin):
...
class PPTPConnection(BASEV2, HasId, HasTenant, TimestampMixin):
...
class L3HARouterAgentPortBinding(BASEV2):
...
class L3HARouterNetwork(BASEV2, HasId, HasTenant):
...
class PortForwardingRule(BASEV2, HasId):
    router_id = sa.Column(sa.String(36),
                          sa.ForeignKey('routers.id',
                                        ondelete="CASCADE"))
