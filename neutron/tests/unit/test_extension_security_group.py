...
from neutron.extensions import _uos_sgrule_default_cfg as default_sg_cfg
...
class TestSecurityGroups(SecurityGroupDBTestCase):
    def test_create_security_group(self):
        ...
        self.assertGreaterEqual(len(sg_rules), 2)
        #NOTE(WeiWang) We have more sg rules in default

        v4_rules = [r for r in sg_rules if r['ethertype'] == const.IPv4]
        self.assertGreaterEqual(len(v4_rules), 1)
        #NOTE(WeiWang) We have more sg rules in default
        v4_rule = v4_rules[0]
    ...
    def test_create_security_group_uos(self):
        DefaultSGRules = ('ingress,icmp,8,0:ingress,tcp,80,80:' +
                          'ingress,tcp,443,443:ingress,self,0,0:' +
                          'ingress,self,0,0')
        cfg.CONF.set_override('securitygroup_default_rules',
                              DefaultSGRules, 'unitedstack')
        default_sg_cfg.DefaultSGRulesConfig.get_valid_rules()
        if 'IPv6' in ext_sg.sg_supported_ethertypes:
            ext_sg.sg_supported_ethertypes.remove('IPv6')
        name = 'webservers'
        description = 'my webservers'
        keys = [('name', name,), ('description', description)]
        with self.security_group(name, description) as security_group:
            pass
        default_sg_cfg.DefaultSGRulesConfig.sg_default_rules = []
        for k, v, in keys:
            self.assertEqual(security_group['security_group'][k], v)

        sg_rules = security_group['security_group']['security_group_rules']
        self.assertEqual(len(sg_rules), 6)

        v4_rules = [r for r in sg_rules if r['ethertype'] == const.IPv4]
        self.assertEqual(len(v4_rules), 6)
        expected_egress = {'direction': 'egress',
                           'remote_group_id': None,
                           'remote_ip_prefix': None,
                           'protocol': None,
                           'port_range_max': None,
                           'port_range_min': None}
        expected_icmp = {'direction': 'ingress',
                         'ethertype': const.IPv4,
                         'remote_group_id': None,
                         'remote_ip_prefix': None,
                         'protocol': "icmp",
        ...
        expected_80 = {'direction': 'ingress',
                       ...
                       'port_range_max': 80,
                       'port_range_min': 80}
        expected_443 = {'direction': 'ingress',
                        'ethertype': const.IPv4,
                        'remote_group_id': None,
                        'remote_ip_prefix': None,
                        'protocol': "tcp",
                        'port_range_max': 443,
                        'port_range_min': 443}
        expected_3389 = {'direction': 'ingress',
                         'ethertype': const.IPv4,
                         'remote_group_id': None,
                         'remote_ip_prefix': None,
                         'protocol': "tcp",
                         ...
                         'port_range_min': 3389}
        for k in sg_rules:
            if k['direction'] == 'egress':
                self._assert_sg_rule_has_kvs(k, expected_egress)
            if k['port_range_max'] == 0:
                self._assert_sg_rule_has_kvs(k, expected_icmp)
            if k['port_range_max'] == 22:
                self._assert_sg_rule_has_kvs(k, expected_22)
            if k['port_range_max'] == 80:
                self._assert_sg_rule_has_kvs(k, expected_80)
            if k['port_range_max'] == 443:
                self._assert_sg_rule_has_kvs(k, expected_443)
            if k['port_range_max'] == 3389:
                self._assert_sg_rule_has_kvs(k, expected_3389)

    def test_create_security_group_uos_3(self):
        DefaultSGRules = ('ingress,self,0,0:ingress,tcp,80,80')
        cfg.CONF.set_override('securitygroup_default_rules',
                              DefaultSGRules, 'unitedstack')
        ...
        if ('default' == groups['security_groups'][0]['name']):
            group = groups['security_groups'][0]
        else:
            group = groups['security_groups'][1]

        sg_rules = group['security_group_rules']
        self.assertEqual(len(sg_rules), 3)
        expected_ingress = {'direction': 'ingress',
                       'remote_ip_prefix': None,
        ...
        for rule in sg_rules:
            if rule['remote_group_id'] is not None:
                self._assert_sg_rule_has_kvs(rule, expected_ingress)

    def test_create_security_group_uos_2(self):
        DefaultSGRules = ('ingress,icmp,8,0:ingress,tcp,80,80:' +
                          'ingress,tcp,443,443:ingress,self,0,0')
        cfg.CONF.set_override('securitygroup_default_rules',
                              DefaultSGRules, 'unitedstack')
        ...
        sg_rules = security_group['security_group']['security_group_rules']
        self.assertEqual(len(sg_rules), 5)

        v4_rules = [r for r in sg_rules if r['ethertype'] == const.IPv4]
        self.assertEqual(len(v4_rules), 5)
        ...
        expected_icmp = {'direction': 'ingress',
                         ...
                         'port_range_max': 0,
                         'port_range_min': 8}
        expected_self = {'direction': 'ingress',
                         'ethertype': const.IPv4,
        ...
        for k in sg_rules:
            ...
            if k['port_range_max'] == 0:
                self._assert_sg_rule_has_kvs(k, expected_icmp)
            if k['port_range_max'] is None and k['direction'] == 'ingress':
                self._assert_sg_rule_has_kvs(k, expected_self)
            if k['port_range_max'] == 80:
                self._assert_sg_rule_has_kvs(k, expected_80)
            if k['port_range_max'] == 443:
                self._assert_sg_rule_has_kvs(k, expected_443)
        ...
        with self.security_group(name, description) as sg:
            ...
            with self.security_group_rule(security_group_id, direction,
                ...
                self.assertEqual(group['security_group']['id'],
                                 remote_group_id)
                self.assertGreaterEqual(len(sg_rule), 3)
    ...
    def test_create_security_group_rule_group_id(self):
        ...
        description = 'my webservers'
        with self.security_group(name, description) as sg:
            with self.security_group(name2, description) as sg2:
                security_group_id = sg['security_group']['id']
                direction = "ingress"
    ...
    def test_create_security_group_rule_different_security_group_ids(self):
        if self._skip_native_bulk:
            self.skipTest("Plugin does not support native bulk "
                          "security_group_rule create")
        with self.security_group() as sg1:
            with self.security_group(name="webservers2") as sg2:
                rule1 = self._build_security_group_rule(
