...
class TestPortsV2(NeutronDbPluginV2TestCase):
    ...
    def test_create_port_fixed_ips_limited(self):
        cfg.CONF.set_override('max_fixed_ips_per_port', 1)
        with self.network() as network:
            with self.subnet(network=network, cidr='10.0.0.0/24') as subnet:
                keys = {'fixed_ips': [{'subnet_id': subnet['subnet']['id'],
                                      'ip_address': '10.0.0.2'},
    ...
    def test_create_service_port_fixed_ips_limited(self):
        cfg.CONF.set_override('max_fixed_ips_per_port', 1)
        with self.network() as network:
            with self.subnet(network=network, cidr='10.0.0.0/24') as subnet:
                keys = {'fixed_ips': [{'subnet_id': subnet['subnet']['id'],
...
""".strip() % (fixed_ips['ip_address'],
    ...
    def test_update_port_ip_limited(self):
        cfg.CONF.set_override('max_fixed_ips_per_port', 1)
        with self.subnet() as subnet:
            with self.port(subnet=subnet) as port:
                data = {'port': {'fixed_ips': [{'subnet_id':
                                               ...
                                               {'subnet_id':
                                                subnet['subnet']['id']}]}}
                self._update('ports', port['port']['id'],
                        data, webob.exc.HTTPBadRequest.code)

    def test_update_service_port_ip_unlimited(self):
        cfg.CONF.set_override('max_fixed_ips_per_port', 1)
        with self.subnet() as subnet:
            with self.port(subnet=subnet) as port:
                data = {'port': {'device_owner': 'manila:share'}}
                self._update('ports', port['port']['id'],
                        data, webob.exc.HTTPOk.code)
                data = {'port': {'fixed_ips': [{'subnet_id':
                                                    subnet['subnet']['id'],
                                                'fixed_ip':'10.0.0.2'},
                                               {'subnet_id':
