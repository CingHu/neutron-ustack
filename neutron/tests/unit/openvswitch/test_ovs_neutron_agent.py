...
class TestOvsNeutronAgent(base.BaseTestCase):
    ...
    def test_setup_physical_bridges_using_veth_interconnection(self):
        ...
        with contextlib.nested(
            ...
            self.assertEqual(self.agent.phys_ofports["physnet1"],
                             "int_ofport")
            if cfg.CONF.USTACK.reset_ovs is False:
                self.assertEqual(remflows_fn.call_count, 0)
                self.assertEqual(ovs_delport_fn.call_count, 0)
    ...
    def test_setup_tunnel_br(self):
        ...
        with contextlib.nested(
            ...
            if cfg.CONF.USTACK.reset_ovs is False:
                self.assertEqual(remove_all_fn.call_count, 0)
