...
class OvsAgentSchedulerTestCase(OvsAgentSchedulerTestCaseBase):
    ...
    def test_network_scheduler_with_down_agent(self):
        ...
        agent_id = self._get_agent_id(dhcp_hosta['agent_type'], dhcp_hosta['host'])
        new_agent = {}
        new_agent['agent'] = {}
        new_agent['agent']['reserved'] = True
        self._update('agents', agent_id, new_agent)
        #NOTE(WeiWang) we allow agent available when it "is" down
        with mock.patch(is_agent_down_str) as mock_is_agent_down:
    ...
    def test_router_reschedule_from_dead_agent(self):
        with self.router():
            ...
            agent = self._list_l3_agents_hosting_router(ret_a[0]['id'])['agents'][0]
            new_agent = {}
            new_agent['agent'] = {}
            new_agent['agent']['reserved'] = True
            self._update('agents', agent['id'], new_agent)
            #NOTE(WeiWang) we allow agent available when it "is" down
