...
portforwarding_opts = [
    # TODO(wwang): use quota framework when it support quota for attributes
    cfg.IntOpt('quota_portforwarding', default=10,
               help=_("Maximum number of port forwaarding per router")),
]

cfg.CONF.register_opts(portforwarding_opts)


class RouterPortForwardingNotFound(qexception.InvalidInput):
    message = _("Port forwarding rule not found in router %(router_id)s.")


# Duplicated Outside Port Exceptions
class DuplicatedOutsidePort(qexception.InvalidInput):
    message = _("Outside port has already been used.")


# Duplicated Outside Port Exceptions
class DuplicatedInsidePort(qexception.InvalidInput):
    message = _("Inside address and port have already been used.")
...
class OverQuota(qexception.Conflict):
    message = _("Quota exceeded for portforwarding, which allows %(quota)s "
                "and used %(usage)s")
