...
class SecurityGroupDuplicateName(qexception.InvalidInput):
    message = _("Security group '%(name)s' already exists.")
...
sg_supported_ethertypes = set(const.sg_supported_ethertypes)
...
security_group_support_opts = [
    cfg.BoolOpt('sg_ipv6_support',
                default=True,
                help=_('To support IPV6 in security group.')),
]
cfg.CONF.register_opts(security_group_support_opts, 'SECURITYGROUP')
    ...
    def get_resources(cls):
        ...
        for resource_name in ['security_group', 'security_group_rule']:
            ...
            ex = extensions.ResourceExtension(collection_name,
                                              ...
                                              attr_map=params)
            exts.append(ex)
        if cfg.CONF.SECURITYGROUP.sg_ipv6_support:
            sg_supported_ethertypes.add('IPv6')
        return exts
