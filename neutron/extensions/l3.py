...
class ExternalGatewayForFloatingIPNotFound(qexception.NotFound):
    message = _("There is no gateway in the router %(router_id)s which "
                "the port %(port_id)s connects. Therefore, "
                "cannot associate with a Floating IP.")
...
class RouterConnectToSubnetNotFound(qexception.NotFound):
    message = _("There are no routers connected to the subnet %(subnet_id)s "
                "which the appointed Fixed IP of the port %(port_id)s "
                "associates with. Therefore, cannot associate with a "
                "Floating IP.")
...
RESOURCE_ATTRIBUTE_MAP = {
    ...
    'floatingips': {
        ...
        'floating_subnet_id': {'allow_post': True, 'allow_put': False,
...
l3_quota_opts = [
    ...
    cfg.IntOpt('quota_floatingip',
               ...
               help=_('Number of floating IPs allowed per tenant. '
                      'A negative value means unlimited.')),
    cfg.IntOpt('quota_portforwardings',
               default=10,
               help=_('Number of portforwardings allowed per tenant. '
                      'A negative value means unlimited.')),
    cfg.IntOpt('quota_router_routes_count',
               default=5,
               help=_('Number of router routes allowed per tenant. '
                      'A negative value means unlimited.')),
]
...
class L3(extensions.ExtensionDescriptor):
    ...
    def get_resources(cls):
        ...
        quota.QUOTAS.register_resource_by_name('router_routes_count')
