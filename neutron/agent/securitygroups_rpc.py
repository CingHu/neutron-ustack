...
class SecurityGroupAgentRpcMixin(object):
    ...
    def prepare_devices_filter(self, device_ids, device_port_dict=None):
        if not device_ids:
            return
        ...
        with self.firewall.defer_apply():
            for device in devices.values():
                if device_port_dict and device['device'] in device_port_dict:
                    device['profile'] = (device_port_dict[device['device']].
                                         get('profile', {}))
    ...
    def refresh_firewall(self, device_ids=None, device_port_dict=None):
    ...
    def setup_port_filters(self, new_devices, updated_devices,
        ...
        if new_devices:
            LOG.debug(_("Preparing device filters for %d new devices"),
                      len(new_devices))
            self.prepare_devices_filter(new_devices, device_port_dict)
        ...
        if global_refresh_firewall:
            ...
            if updated_devices:
                LOG.debug(_("Refreshing firewall for %d devices"),
                          len(updated_devices))
                self.refresh_firewall(updated_devices, device_port_dict)
