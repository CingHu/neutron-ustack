...
class L3AgentMixin(object):
    ...
    def _uos_get_ex_gw_port(self, ri):
        ...
        if ri.uos_gateway_ip in gateway_ips:
            gw_port['subnet']['gateway_ip'] = ri.uos_gateway_ip
        elif gateway_ips:
            gw_port['subnet']['gateway_ip'] = gateway_ips[0]
        return gw_port
    ...
    def _create_ratelimit_qdisc(self, ip_wrapper, ri, interface_name):
        ...
        qdisc_count = self.conf.default_tc_qdisc
        self._add_class_filter(
            ip_wrapper, default_class, interfaces, 1024*qdisc_count*100)
    ...
    def process_metering_label(self, ri):
        ...
        not_metering = ri.iptables_manager.ipv4['filter'].is_chain_empty(TOP_CHAIN)
        ...
        LOG.info('changzhi router %s get fips from server', ri.router_id)
        ...
        LOG.info('changzhi router %s get fips from server end', ri.router_id)
        for fip in ri.router.get(l3_constants.UOS_GW_FIP_KEY, []):
            fips.append({'floating_ip_address':
                         fip['floating_ip_address'],
                         'fixed_ip':fip['fixed_ip_address'],
        ...
        LOG.info('changzhi router %s get uos fip end', ri.router_id)
        ...
        try:
            ...
            for fip in fips:
                ...
                fip_not_metering = ri.iptables_manager.ipv4['filter'].is_chain_empty(fip_chain_name)

                #sync all floatingips and store in available_fips
                available_fips_set.add(fip['id'])

                if fip_not_metering:#not exist, so generate iptables rule
                    self.create_fip_chain_and_rule(ip_wrapper, ri, fip, qg_dev)

            LOG.info('changzhi router %s manage fips done', ri.router_id)
            ...
            if ri.lock.acquire():
                ...
                ri.lock.release()
                #unlock!
            LOG.info('changzhi router %s lock release', ri.router_id)
            ...
            LOG.info('changzhi router %s metering end', ri.router_id)
