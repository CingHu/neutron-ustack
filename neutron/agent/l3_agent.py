...
from neutron.agent import uos_l3
...
class L3PluginApi(n_rpc.RpcProxy):
    ...
    def get_routers(self, context, router_ids=None):
        ...
        _rts = self.call(context,
                         self.make_msg('sync_routers', host=self.host,
                                       router_ids=router_ids))
        if LOG_UOS.logger.isEnabledFor(10):
            LOG_UOS.debug("get_routers...\n%s",
                          jsonutils.dumps(_rts, indent=5))
        return _rts
...
class RouterInfo(object):
    ...
    def __init__(self, router_id, root_helper, use_namespaces, router,
        ...
        # to remember which gateway_ip is using
        self.uos_gateway_ip = None
        self.portforwardings = []
        self.fip_class_ratelimit_dict = {}
        self.available_classes = set(xrange(11, 65530))
        # to remember floatingips in this router
        self.fip_metering_set = set()
        self.uos_gateway_fip = None
        self.all_fips = {}
...
class L3NATAgent(firewall_l3_agent.FWaaSL3AgentRpcCallback, manager.Manager,
    ...
    OPTS = [
        ...
        cfg.StrOpt('metadata_proxy_socket',
                   ...
                   help=_('Location of Metadata Proxy UNIX domain '
                          'socket')),
        cfg.BoolOpt('l3_fip_in_qos',
                    default=False,
                    help=_('Enable to limit the input traffic.')),
        cfg.IntOpt('metering_interval',
                   default=60,
                   help=_("Seconds between reporting traffic.")),
        cfg.BoolOpt('uos_metering',
                    default=False,
                    help=_('Enable to floatingip traffic.')),
        cfg.IntOpt('notification_count',
                   default=10,
                   help=_("Counts for notice ceilometer.")),
        cfg.IntOpt('default_tc_qdisc',
                   default=5,
                   help=_("Default value for tc qdisc.")),
    ]
    ...
    def __init__(self, host, conf=None):
        ...
        #NOTE(gongysh) It seemed, the get service plugins cannot work here
    ...
    def _router_added(self, router_id, router):
        ...
        # For router_added we should not call iptables_manager
        # apply for it will cause the iptables rule refresh
    ...
    def process_router(self, ri):
        ...
        try:
            if ex_gw_port:
                ...
                LOG.info('changzhi router %s self.process_router_floating_ip_nat_rules', ri.router_id)
                self.process_router_floating_ip_nat_rules(ri, ex_gw_port)
                LOG.info('changzhi router %s self.process_router_floating_ip_nat_rules end', ri.router_id)
                self.process_router_floating_ip_ratelimit_rules(ri, ex_gw_port)
                ri.iptables_manager.defer_apply_off()
                LOG.info('changzhi router %s self.process_router_floating_ip_addresses', ri.router_id)
                # Once NAT rules for floating IPs are safely in place
                # configure their addresses on the external gateway port
                fip_statuses = self.process_router_floating_ip_addresses(
                    ri, ex_gw_port)
                LOG.info('changzhi router %s self.process_router_floating_ip_addresses end', ri.router_id)
        except Exception:
    ...
    def process_router_floating_ip_nat_rules(self, ri, ex_gw_port=None):
        ...
        for fip in floating_ips:
            ...
            for chain, rule in self.floating_forward_rules(fip_ip, fixed):
                ri.iptables_manager.ipv4['nat'].add_rule(chain, rule,
                                                         tag='floating_ip')
        if ex_gw_port:
            if cfg.CONF.uos_metering and cfg.CONF.metering_interval:
                LOG.info('changzhi router %s start process_metering_label', ri.router_id)
                self.process_metering_label(ri)
                LOG.info('changzhi router %s end process_metering_label', ri.router_id)
        ri.iptables_manager.apply()

    def process_router_floating_ip_ratelimit_rules(self, ri, ex_gw_port=None):
        ...
        if ex_gw_port:
            ...
            LOG.info('changzhi router %s start process_rate_limit', ri.router_id)
            self.process_rate_limit(ri, interface_name)
    ...
    def process_router_floating_ip_addresses(self, ri, ex_gw_port):
        ...
        for fip in floating_ips:
            ...
            if ip_cidr not in existing_cidrs:
                ...
                if ri.router['distributed']:
                    ...
                    # self.floating_ip_added_for_policy_route(ri, fip, interface_name)
        ...
        for ip_cidr in existing_cidrs - new_cidrs:
            if ip_cidr.endswith(FLOATING_IP_CIDR_SUFFIX):
                ...
                #else:
                    #self.floating_ip_deleted_for_policy_route(ri, ip_cidr, interface_name)

        return fip_statuses

    def _get_ex_gw_port(self, ri):
        return self._uos_get_ex_gw_port(ri)
    ...
    def _external_gateway_added(self, ri, ex_gw_port, interface_name,
        ...
        if call_create_ratelimit_qdisc:
            LOG.info('router %s _create_ratelimit_qdisc called', ri.router_id)
            self._create_ratelimit_qdisc(ip_wrapper, ri, interface_name)
        self.driver.init_l3(interface_name, [ex_gw_port['ip_cidr']],
                            namespace=ns_name,
                            gateway=ex_gw_port['subnet'].get('gateway_ip'),
                            extra_subnets=ex_gw_port.get('extra_subnets', []),
                            preserve_ips=preserve_ips)
        ri.uos_gateway_ip = ex_gw_port['subnet'].get('gateway_ip')
    ...
    def external_gateway_removed(self, ri, ex_gw_port, interface_name):
        ...
        ip_wrapper = ip_lib.IPWrapper(self.root_helper,
                                      namespace=ri.ns_name)
        self._delete_ratelimit_qdisc(ip_wrapper, interface_name)
        if ri.router['distributed']:
    ...
    def _change_routing_table(self, ri, operation, nexthop , table_id):
        if nexthop:
            cmd = ['ip', 'route', operation,'table', table_id, 'to', '0.0.0.0/0',
               'via', nexthop]
        else:
            cmd = ['ip', 'route', operation,'table', table_id]
        ip_wrapper = ip_lib.IPWrapper(self.root_helper,
                                      namespace=ri.ns_name)
        ip_wrapper.netns.execute(cmd, check_exit_code=False)

    def floating_ip_added_for_policy_route(self, ri, fip, interface_name):
        """Add floating IP policy route."""
        floating_ip = fip['floating_ip_address']
        floating_ip_array = floating_ip.split('.')
        floating_ip_string = 1
        for i in range(0, 4):
            floating_ip_string = int(floating_ip_array[i]) * floating_ip_string
        LOG.info(_("floating_ip_added_for_policy_route"
                   " fip '%(fip)s',gw '%(gateway_ip)s',"
                   "lookup '%(fip_string)s'") % {
                       'fip' : floating_ip,
                       'gateway_ip' : fip['gateway_ip'],
                       'fip_string' : floating_ip_string
                   })
        self._change_routing_table(ri, 'replace', nexthop=fip['gateway_ip'],
                                     table_id=floating_ip_string)
        ip_rule = ip_lib.IpRule(self.root_helper, namespace=ri.ns_name)
        result = ip_rule.add_rule_from_with_check(floating_ip, floating_ip_string, 10)

    def floating_ip_deleted_for_policy_route(self, ri, fip, interface_name):
        """Delete floating IP policy route."""
        floating_ip = fip
        floating_ip_array = floating_ip.split('/')[0].split('.')
        floating_ip_string = 1
        for i in range(0, 4):
            floating_ip_string = int(floating_ip_array[i]) * floating_ip_string
        LOG.info(_("floating_ip_deleted_for_policy_route"
                   " fip '%(fip)s',lookup '%(fip_string)s' ") % {
                       'fip' : floating_ip,
                       'fip_string' : floating_ip_string
                     })
        ip_rule = ip_lib.IpRule(self.root_helper, namespace=ri.ns_name)
        ip_rule.del_rule_from(floating_ip, floating_ip_string, 10)
        self._change_routing_table(ri, 'flush',nexthop=None,table_id=floating_ip_string)
    ...
    def routers_updated(self, context, routers):
        ...
        LOG.info(_('Got routers updated notification :%s'), routers)
        if routers:
    ...
    def _process_router_update(self):
        for rp, update in self._queue.each_update_to_next_router():
            LOG.info("Starting router update for %s", update.id)
...
class L3NATAgentWithStateReport(L3NATAgent):
    ...
    def __init__(self, host, conf=None):
        ...
        #(NOTE)changzhi add traffic metering
        if cfg.CONF.metering_interval and cfg.CONF.uos_metering:
            self.metering = loopingcall.FixedIntervalLoopingCall(
                self.traffic_metering)
            self.metering.start(interval=cfg.CONF.metering_interval)

    def traffic_metering(self):
        LOG.debug(_("Report traffic started."))

        count = cfg.CONF.notification_count

        try:
            router_infos = self.router_info.values()
            data = self.get_all_fips_traffic(router_infos)
            notifier = n_rpc.get_notifier('metering')

            for i in range(0, len(data), count):
                send_data = data[i:count+i]
                LOG.info(_("Send metering report %s" % send_data))
                LOG.info(_("Send metering report len %s" % len(send_data)))
                notifier.info(self.context, 'l3_agent.metering', send_data)

        except Exception:
            LOG.error('Exception has happed when traffic_metering start')
            LOG.error(traceback.format_exc())
            LOG.error('Exception has happed when traffic_metering end')
