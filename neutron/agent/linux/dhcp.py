...
OPTS = [
    ...
    cfg.BoolOpt('kill_dhcp_process', default=False,
        help='Kill dhcp process when restart dhcp agent'),
]
...
class DhcpBase(object):
    ...
    @abc.abstractmethod
    def enable(self, kill_flag=True):
    ...
    def restart(self, **kwargs):
        ...
        kill_flag = kwargs.get('kill_flag', None)

        if kill_flag == False:
            self.reload_allocations()
            return

        LOG.debug('call kill and enable flag')
        self.disable(retain_port=True)
        self.enable()
...
class DeviceManager(object):
    ...
    def setup_dhcp_port(self, network):
        ...
        fixed_ips = [dict(subnet_id=fixed_ip.subnet_id,
                     ...
                     for fixed_ip in dhcp_port.fixed_ips if subnets.get(
                             fixed_ip.subnet_id, None)]
