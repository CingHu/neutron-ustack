...
class IptablesFirewallDriver(firewall.FirewallDriver):
    ...
    def _remove_chains_apply(self, ports):
        for port in ports.values():
            ...
            self._remove_chain(port, DDOS_FILTER)
    ...
    def _add_chain(self, port, direction):
        ...
        # mark syn packet
        if "o" == CHAIN_NAME_PREFIX[direction]:
            profile = port.get('profile', {})
            uos_marks = profile.get('uos_mark_actions')
            if uos_marks is None:
                uos_marks = []
            for uos_mark in uos_marks:
                # '-p tcp --syn -j MARK --set-mark 1'
                syn_mark_rule = ['-m physdev --%s %s --physdev-is-bridged %s' %
                                 (self.IPTABLES_DIRECTION[direction], device,
                                  uos_mark)]
                self._add_rule_to_chain_v4v6(SG_CHAIN, syn_mark_rule,
                                             syn_mark_rule)
    ...
    def _setup_ddos_filter_chain(self, port, table, rules):

        profile = port.get('binding:profile', {})
        uos_pps_limits = profile.get('uos_pps_limits')
        chain_name = self._port_chain_name(port, DDOS_FILTER)
        table.add_chain(chain_name)
        if uos_pps_limits is None:
            uos_pps_limits = []

        limit_rules = []
        for uos_pps_limit in uos_pps_limits:
            uos_pps_rule_array = uos_pps_limit.split(':')
            if uos_pps_rule_array[1]:
                type_limit = (' --'+uos_pps_rule_array[1])
            else:
                type_limit = ''
            if uos_pps_rule_array[2]:
                port_limit = (' --dport '+ uos_pps_rule_array[2])
            else:
                port_limit = ''
            #to make the name not same with each other
            hash_name = (port['device'][:8]
                        + str(uuid.uuid5(uuid.NAMESPACE_DNS,
                               str(uos_pps_limit))))

            if (int(uos_pps_rule_array[3])>10000):
                uos_pps_rule_array[3] = str(10000)

            burst_limit_count = int(uos_pps_rule_array[3])*2
            burst_limit_str = str(burst_limit_count)
            if (burst_limit_count>10000):
                burst_limit_str = str(10000)

            limit_rule = (' -p ' + uos_pps_rule_array[0]
                         + type_limit
                         + port_limit
                         + ' -m hashlimit'
                         + ' --hashlimit-mode srcip'
                         + ' --hashlimit-above '
                         + uos_pps_rule_array[3] +
                         '/second'
                         + ' --hashlimit-burst '
                         + burst_limit_str
                         +' --hashlimit-name '
                         + hash_name + ' -j DROP')
            limit_rules.append(limit_rule)
    ...
    def _setup_spoof_filter_chain(self, port, table, mac_ip_pairs, rules):
        ...
        # NOTE(changzhi) Get port's anti-spoofing flag. If flag is True, means
        # this port can send pkgs by any ip or mac. So just add a spoof chain
        # and return. If flag is False, go on by originl code.
        if mac_ip_pairs:
            spoofing = port.get('disable_anti_spoofing', None)
            if spoofing == True:
    ...
    def _spoofing_rule(self, port, ipv4_rules, ipv6_rules):
        ...
        # NOTE(gongysh) only networks without subnets will have this
        # not to deal with it so that we can support custom network.
        ...
        self._setup_ddos_filter_chain(port, self.iptables.ipv4['filter'],
                                       ipv4_rules)
