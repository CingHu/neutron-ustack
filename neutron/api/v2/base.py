...
class Controller(object):
    ...
    @property
    def _policy_attrs(self):
        return [name for (name, info) in self._attr_info.items()
                if info.get('required_by_policy')]

    @property
    def _attr_info(self):
        # NOTE(gongysh) health monitors are used by top and sub resource
        if self._collection == 'health_monitors':
            return self._attr_info_inner
        self._merge_attr()
        return attributes.RESOURCE_ATTRIBUTE_MAP.get(self._collection)

    def _merge_attr(self):
        _temp = attributes.RESOURCE_ATTRIBUTE_MAP.get(self._collection)
        if not _temp:
            attributes.RESOURCE_ATTRIBUTE_MAP[
                self._collection] = self._attr_info_inner
        else:
            _temp.update(self._attr_info_inner)
    ...
    def __init__(self, plugin, collection, resource, attr_info,
        ...
        self._attr_info_inner = attr_info
    ...
    def create(self, request, body=None, **kwargs):
        ...
        for item in items:
            ...
            policy.enforce(request.context,
                           action,
                           item[self._resource])
            # NOTE(gongysh) sometimes the resource is not owned by tenant
            if 'tenant_id' in item[self._resource]:
                try:
    ...
    def prepare_request_body(context, body, is_create, resource, attr_info,
        ...
        for attr, attr_vals in attr_info.iteritems():
            ...
            if 'validate' not in attr_vals:
                continue
            if isinstance(attr_vals['validate'], types.FunctionType):
                res = attr_vals['validate'](res_dict[attr])
                if res:
