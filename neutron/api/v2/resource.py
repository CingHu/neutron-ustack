...
def translate(translatable, locale):
    ...
    if isinstance(translatable, exceptions.NeutronException):
        localized_message = localize(translatable.message, locale)
        translatable.msg = localized_message % translatable.kwargs
...
def file_response(request, status, content_type, content_disposition, contents):
    res =  webob.Response(request=request, status=status,
                              content_type=content_type,
